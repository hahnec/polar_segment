# adaption of https://github.com/gatsby2016/Augmentation-PyTorch-Transforms
...
class ToPILImage(object):
    ...
    def __call__(self, img, label=None, *args, **kwargs):
        """
        Args:
            img (Tensor or numpy.ndarray): Image to be converted to PIL Image.
        ...
        if label is not None:
            if label.dtype != np.uint8: label = label.astype(np.uint8)
            return F.to_pil_image(img.squeeze(), self.mode), F.to_pil_image(label.squeeze(), self.mode)
        return F.to_pil_image(img.squeeze(), self.mode)
...
class Normalize(object):
    ...
    def __call__(self, tensor, label=None, *args, **kwargs):
        """
        Args:
        ...
        """
        if label is not None:
            return F.normalize(tensor, self.mean, self.std, self.inplace), F.normalize(label, self.mean, self.std, self.inplace)
...
class RandomApply(RandomTransforms):
    ...
    def __call__(self, img, label=None, *args, **kwargs):
        ...
        if label is not None:
            for t in self.transforms:
                img, label = t(img, label, *args, **kwargs)
            return img, label
        else:
            for t in self.transforms:
...
class RandomOrder(RandomTransforms):
    ...
    """
    def __call__(self, img, label, *args, **kwargs):
        order = list(range(len(self.transforms)))
        random.shuffle(order)
        for i in order:
            img = self.transforms[i](img)
            if label is not None: label = self.transforms[i](label)
        if label is not None:
            return img, label
        return img
...
class RandomPerspective(object):
    ...
    def __call__(self, img, label=None, *args, **kwargs):
        ...
        if random.random() < self.p:
            ...
            if label is not None:
                return F.perspective(img, startpoints, endpoints, self.interpolation), F.perspective(label, startpoints, endpoints, self.interpolation)
...
class RandomRotation(object):
    ...
    def __init__(self, degrees, resample=False, expand=False, center=None, fill=0, p=0.5):
...
class RandomAffine(object):
    ...
    def __call__(self, img, label=None, *args, **kwargs):
        ...
        if label is not None:
            label = F.affine(label, *ret, resample=self.resample, fillcolor=self.fillcolor)
            return img, label
        return img
...
class HEDJitter(object):
    ...
    def __call__(self, img, label=None, *args, **kwargs):
        img = self.adjust_HED(img, self.alpha, self.betti)
...
class AutoRandomRotation(object):
    ...
    def __init__(self, degree=None, resample=False, expand=True, center=None, fill=0, p=0.5):
        if degree is None:
    ...
    def __call__(self, img, label=None, *args, **kwargs):
        ...
        if random.random() < self.p:
            img = F.rotate(img, self.degrees, self.resample, self.expand, self.center, self.fill)
            if label is not None:
                label = F.rotate(label, self.degrees, self.resample, self.expand, self.center, self.fill)
                return img, label
            return img
        else:
            if label is not None:
...
class RandomGaussBlur(object):
    ...
    """
    def __init__(self, radius=None, p=0.5):
    ...
    def __call__(self, img, label=None, *args, **kwargs):
        ...
        if random.random() < self.p:
            ...
            if label is not None:
                label = label.filter(ImageFilter.GaussianBlur(radius=self.radius))
                return img, label
            return img
        else:
            if label is not None:
...
class RandomAffineCV2(object):
    ...
    def __call__(self, img, mask=None, label=None, *args, **kwargs):
        img = self.affineTransformCV2(np.array(img), self.alpha, mask)
    
        if label is not None:
            label = self.affineTransformCV2(np.array(label), self.alpha, mask)
            return img, label
        return img
...
class RandomElastic(object):
    ...
    @staticmethod
    def RandomElasticCV2(img, alpha, sigma, xrand, yrand, mask=None):
        alpha = img.shape[1] * alpha
        ...
        dx = gaussian_filter((xrand * 2 - 1), sigma) * alpha
        dy = gaussian_filter((yrand * 2 - 1), sigma) * alpha
        # dz = np.zeros_like(dx)
        ...
        if mask is not None:
            return Image.fromarray(img[..., :3].squeeze()), Image.fromarray(img[..., 3].squeeze())
        else:
            return Image.fromarray(img.squeeze())

    def __call__(self, img, label=None, mask=None, *args, **kwargs):

        if random.random() < self.p:
            shape = np.array(img).shape
            if len(shape) == 2: shape, img = (*shape, 1), np.array(img)[..., None]
            xrand = np.random.rand(*shape)
            yrand = np.random.rand(*shape)
            img = self.RandomElasticCV2(np.array(img), self.alpha, self.sigma, xrand, yrand, mask)
            if label is not None:
                label = self.RandomElasticCV2(np.array(label), self.alpha, self.sigma, xrand, yrand, mask)
                return img, label
            return img
        else:
            if label is not None:
