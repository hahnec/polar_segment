...
def draw_segmentation_imgs(imgs, preds, truth, bidx=0, th=None, alpha=0.5):
    ...
    if th is None:
        combined_masks = torch.stack((preds[bidx, 1]>preds[bidx, 0], preds[bidx, 0]>preds[bidx, 1], truth[bidx, 1]>0, truth[bidx, 0]>0)).cpu()
    else:
        combined_masks = torch.stack((preds[bidx, 1]>th[0][1], preds[bidx, 0]>th[0][0], truth[bidx, 1]>0, truth[bidx, 0]>0)).cpu()
