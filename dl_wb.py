...
if __name__ == "__main__":
    ...
    group_stem = 'tip_mm_bs4_b' if not project_name.__contains__('test') else 'test_run'
    table_key = 'report'
    media_keys = ['heatmap_test', 'img_pred_test', 'img_mask_test', 'img_fiber_test']

    api = wandb.Api()

    for run_suffix in ['absence', 'noise', 'rota', 'flip', 'rotaflip']:
  
        group_name = group_stem + '_' + run_suffix
