...
def epoch_iter(cfg, dataloader, model, mm_model=None, branch_type='test', step=None, log_img=False, epoch=None, optimizer=None, grad_scaler=None):
    ...
    criterion = criterion = nn.CrossEntropyLoss() #(lambda x, y: sigmoid_focal_loss_multiclass(x, y, alpha=cfg.alpha, gamma=cfg.gamma).mean()) if branch_type != 'test' else None
    if cfg.class_num > 3 and branch_type != 'test':
        from utils.multi_loss import multi_loss_aggregation
        criterion = criterion = nn.CrossEntropyLoss() #lambda x, y: multi_loss_aggregation(x, y, loss_fun=lambda x, y: sigmoid_focal_loss_multiclass(x, y, alpha=cfg.alpha, gamma=cfg.gamma).mean())
