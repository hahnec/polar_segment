...
def batch_iter(frames, truth, cfg, model, train_opt=0, criterion=None, optimizer=None, grad_scaler=None, gradient_clipping=1.0):
    ...
    for i in range(truth.shape[0]):
        for c in range(truth.shape[1]):
            ...
            ious[i, c] = compute_iou(preds_b[i, c], truth[i, c], mask=mask[i]).detach()
            accs[i, c] = compute_accuracy(preds_b[i, c], truth[i, c], mask=mask[i]).detach()
            dices[i, c] = compute_dice_score(preds_b[i, c], truth[i, c], mask=mask[i]).detach()
        ...
        for batch in dataloader:
            ...
            if cfg.logging and branch_type == 'test':
                for bidx in range(truth.shape[0]):
                    ...
                    hmask = ~(truth[bidx, 0].detach()) if cfg.bg_opt else torch.any(truth[bidx], dim=0)
